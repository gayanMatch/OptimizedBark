...
class GPT2Converter(ModelFileConverter):
    ...
    def _load_history_prompt(self, history_prompt_input):
        ...
        history_prompt = np.load(
            os.path.join('/bark/bark', "assets", "prompts", f"{history_prompt_input}.npz")
        )
    ...
    def torch_to_onnx(
        ...
        device = 'cpu'
        input_ids = torch.from_numpy(np.load('Data/coarse_883.npy'))
        ...
        if not network_metadata.other.kv_cache:
            ...
            def _export_forward(input_ids, **kwargs):
                result = old_forward(input_ids, use_cache = False, merge_context=False, **kwargs)
                return result[0]
            ...
            decoder_output = gpt2_model(input_ids, use_cache = True, merge_context=False)
            past_key_values = decoder_output[1]

            # Exporting the kv cache engine
            old_forward = gpt2_model.forward
            def _export_forward(input_ids, past_key_values, **kwargs):
                result = old_forward(input_ids, past_key_values=past_key_values, use_cache=True, merge_context=False, **kwargs)
                return (result[0], result[1])
            gpt2_model.forward = _export_forward
            input_ids = torch.from_numpy(np.load('Data/coarse_1.npy'))
            torch.onnx.export(
                gpt2_model,
